...
from spatialfeatures2 import DenseNet121

# from spatialfeatures import SFE_DenseNet121
# from spatialfeatures import SFE_DenseNet169
# from spatialfeatures import SFE_Xception
# from spatialfeatures import SFE_ResNet50
...
class BaseModel(object):
    ...
    def __init__(
        ...
        self._set_params_initializer(
            hparams, mode, iterator, target_vocab_table, scope, extra_args
        )
    ...
    def _set_params_initializer(
        self, hparams, mode, iterator, target_vocab_table, scope, extra_args=None
    ):
        ...
        # self.cnn = SFE_DenseNet121(self.cnn_input)
        # self.cnn = SFE_DenseNet169(self.cnn_input)
        # self.cnn = SFE_Xception(self.cnn_input)
        # self.cnn = SFE_ResNet50(self.cnn_input)
        self.cnn = DenseNet121(
            input_shape=self.cnn_input.shape[1:], input_tensor=self.cnn_input
        )
        # Load weights into the CNN graph
        self.cnn.load_weights()

        # if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
        #     self.cnn = AlexNet(
        #         self.cnn_input, (1 - hparams.dropout),
        #         model_helper.get_device_str(hparams.base_gpu, self.num_gpus))
        # else:
        #     self.cnn = AlexNet(
        #         self.cnn_input, 1,
        #         model_helper.get_device_str(hparams.base_gpu, self.num_gpus))
    ...
    def init_embeddings(self, hparams, scope):
        """Init embeddings."""
        self.embedding_decoder = model_helper.create_emb_for_encoder_and_decoder(
...
class Model(BaseModel):
    ...
    def _build_encoder_from_sequence(self, hparams, sequence, sequence_length):
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            self.encoder_emb_inp = self.cnn.output
            print("CNN_Output(shape) : ", self.encoder_emb_inp.shape)
